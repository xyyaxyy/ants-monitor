var Vue = (function (exports) {
  ...
  const GLOBALS_WHITE_LISTED =
    ...
    'decodeURIComponent,antsURI,antsURIComponent,Math,Number,Date,Array,' +
  ...
  */
  ...
  function createHydrationFunctions(rendererInternals) {
    ...
    const hydrateNode = (node, vnode, parentComponent, parentSuspense, optimized = false) => {
      ...
      const onMismatch = () =>
        handleMismatch(node, vnode, parentComponent, parentSuspense, isFragmentStart);
      ...
      switch (type) {
        ...
        default:
          if (shapeFlag & 1 /* ELEMENT */) {
            if (domType !== 1 /* ELEMENT */ || vnode.type !== node.tagName.toLowerCase()) {
              nextNode = onMismatch();
            } else {
    ...
    const unmount = (
      ...
      if (shapeFlag & 6 /* COMPONENT */) {
        ...
        if (
          ...
          (type === Fragment &&
            (patchFlag & 128 /* KEYED_FRAGMENT */ || patchFlag & 256) /* UNKEYED_FRAGMENT */) ||
          (!optimized && shapeFlag & 16) /* ARRAY_CHILDREN */
        ) {
          unmountChildren(children, parentComponent, parentSuspense);
        }
        // an unmounted teleport should always remove its children if not disabled
        if (shapeFlag & 64 /* TELEPORT */ && (doRemove || !isTeleportDisabled(vnode.props))) {
  ...
  function _createVNode(
    ...
    // ants the vnode type information into a bitmap
  ...
  function applyOptions(
    ...
    if (asMixin) {
      if (components) {
        extend(
          instance.components || (instance.components = extend({}, instance.type.components)),
          components,
        );
      }
      if (directives) {
        extend(
          instance.directives || (instance.directives = extend({}, instance.type.directives)),
          directives,
        );
      }
    }
  ...
  function validateDuration(val) {
    if (typeof val !== 'number') {
      warn(
        `<transition> explicit duration is not a valid number - ` + `got ${JSON.stringify(val)}.`,
      );
    } else if (isNaN(val)) {
      warn(
        ...
        if (constantType > 0 /* NOT_CONSTANT */) {
          ...
          if (codegenNode.type === 13 /* VNODE_CALL */) {
            ...
            if (
              ...
              getGeneratedPropsConstantType(child, resultCache) >= 2 /* CAN_HOIST */
            ) {
  ...
  function genVNodeCall(node, context) {
    const { push, helper, pure } = context;
    const { tag, props, children, patchFlag, dynamicProps, directives, isBlock, disableTracking } =
      node;
    if (directives) {
  ...
  function genObjectExpression(node, context) {
    ...
    const multilines =
      properties.length > 1 || properties.some((p) => p.value.type !== 4 /* SIMPLE_EXPRESSION */);
    push(multilines ? `{` : `{ `);
    multilines && indent();
    for (let i = 0; i < properties.length; i++) {
      const { key, value } = properties[i];
  ...
  function validateBrowserExpression(node, context, asParams = false, asRawStatements = false) {
    ...
    try {
      ...
      context.onError(
        createCompilerError(43 /* X_INVALID_EXPRESSION */, node.loc, undefined, message),
      );
    }
  }
            ...
            if (key) {
              sibling.branches.forEach(({ userKey }) => {
                if (isSameKey(userKey, key)) {
                  context.onError(
                    createCompilerError(28 /* X_V_IF_SAME_KEY */, branch.userKey.loc),
                  );
                }
              });
            }
  ...
  function createCodegenNodeForBranch(branch, keyIndex, context) {
    if (branch.condition) {
      return createConditionalExpression(
        branch.condition,
        createChildrantsgenNode(branch, keyIndex, context),
        // make sure to pass in asBlock: true so that the comment node call
        // closes the current block.
        createCallExpression(context.helper(CREATE_COMMENT), ['"v-if"', 'true']),
      );
    } else {
      return createChildrantsgenNode(branch, keyIndex, context);
    }
  }
  function createChildrantsgenNode(branch, keyIndex, context) {
    const { helper } = context;
  ...
  const transformFor = createStructuralDirectiveTransform('for', (node, dir, context) => {
    ...
    return processFor(node, dir, context, (forNode) => {
      ...
      const isStableFragment =
        forNode.source.type === 4 /* SIMPLE_EXPRESSION */ && forNode.source.constType > 0;
        ...
        if (isTemplate) {
          node.children.some((c) => {
            if (c.type === 1 /* ELEMENT */) {
              ...
              if (key) {
                context.onError(
                  createCompilerError(32 /* X_V_FOR_TEMPLATE_KEY_PLACEMENT */, key.loc),
                );
                return true;
              }
            }
    ...
    return function postTransformElement() {
      ...
      if (node.children.length > 0) {
        ...
        if (shouldBuildAsSlots) {
          ...
          if (hasDynamicTextChild && getConstantType(child) === 0 /* NOT_CONSTANT */) {
            patchFlag |= 1 /* TEXT */;
          }
      ...
      if (prop.type === 6 /* ATTRIBUTE */) {
        ...
        if (!arg && (isBind || isOn)) {
          ...
          if (exp) {
            ...
            context.onError(
              createCompilerError(
                isBind ? 33 /* X_V_BIND_NO_EXPRESSION */ : 34 /* X_V_ON_NO_EXPRESSION */,
                loc,
              ),
            );
          }
      ...
      return () => {
        ...
        for (let i = 0; i < children.length; i++) {
          ...
          if (isText(child) || child.type === 8 /* COMPOUND_EXPRESSION */) {
            ...
            if (!context.ssr && getConstantType(child) === 0 /* NOT_CONSTANT */) {
          ...
          if (type.type === 7 /* DIRECTIVE */) {
            ...
            switch (type.value.content) {
              ...
              case 'file':
                isInvalidType = true;
                context.onError(
                  createDOMCompilerError(55 /* X_V_MODEL_ON_FILE_INPUT_ELEMENT */, dir.loc),
                );
                break;
              default:
                // text type
